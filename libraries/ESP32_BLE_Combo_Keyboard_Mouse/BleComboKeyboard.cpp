...
static const uint8_t _hidReportDescriptor[] = {
  ...
  END_COLLECTION(0),         //   END_COLLECTION
  END_COLLECTION(0),          // END_COLLECTION
  //------------------------------------------------- Absolute Pointer (Report ID 4)  
  USAGE_PAGE(1),       0x01, // Generic Desktop
  USAGE(1),            0x02, // Mouse
  COLLECTION(1),       0x01, // Application
  REPORT_ID(1),        ABS_MOUSE_ID,
  ...
  USAGE(1),            0x31, //     Y
  LOGICAL_MINIMUM(1),  0x00,
  LOGICAL_MAXIMUM(2),  0xFF, 0x7F, //  32767  ← 注意是 (2),两个数据字节
  REPORT_SIZE(1),      0x10, //     16 位
  REPORT_COUNT(1),     0x02,
  HIDINPUT(1),         0x02, //     Data,Var,ABSOLUTE
  END_COLLECTION(0),         //   END_COLLECTION
  END_COLLECTION(0)          // END_COLLECTION};
};
...
void BleComboKeyboard::taskServer(void* pvParameter) {
  ...
  bleKeyboardInstance->inputMouseAbs = bleKeyboardInstance->hid->inputReport(ABS_MOUSE_ID);
